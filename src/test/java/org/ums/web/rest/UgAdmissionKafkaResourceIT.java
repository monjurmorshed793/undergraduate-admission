...
import org.ums.config.KafkaProperties;
...
class UgAdmissionKafkaResourceIT {
    ...
    void setup() {
        ...
        UgAdmissionKafkaResource kafkaResource = new UgAdmissionKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ug-admission-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/ug-admission-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
